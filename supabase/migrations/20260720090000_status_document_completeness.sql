-- E2: candidate-facing document completeness ("lengkap/kurang"). The employer
-- board (ApplicationsBoard) already computes this from vacancy.required_documents
-- + application_documents; give the candidate status page (get_application_status)
-- the same two ingredients so the UI can run the identical
-- filterApplicationStageDocuments() completeness check client-side.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
           'type', d.document_type, 'label', d.document_label) order by d.uploaded_at), '[]'::jsonb)
