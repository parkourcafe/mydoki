...
| PR-5 | 1B Hiring | Вакансия: недостающие блоки (§8 v1.1: problem_statement, must_have, trainable, scorecard, stages, success_criteria_probation), draft/published_at, `vacancy_versions` (отклик ссылается на версию), публичная страница `/v/[slug]` |
| PR-6 | 1B Hiring | Отклик: `stage`×`state` (+rejected_reason, withdrawn; совместимо со status для Doki.id), `application_events` (+бэкофилл из status_log), согласие с отзывом (отзыв закрывает доступ компании), мастер отклика из vault с фиксацией `document_version_id` (эволюция `attachVaultDocs`), `/my/applications`: комплектность, лента, отзыв отклика. Судьба гостевого отклика — по решению §9.2 |
| PR-7 | 1B Hiring | Воронка: kanban по stages (`/employer/vacancies/[id]/candidates`) + Candidate Card `/employer/candidates/[applicationId]` (разделы §10 v1.1 без AI-блоков) + `CandidateRequest`/`EmployerNote`/`DecisionRecord`, отказ только с причиной, уведомление кандидата. **Конец вертикали §17 v1.1 — ручной end-to-end процесс работает без AI** |
| PR-8 | 1C AI | Консолидация `lib/{llm,anthropic,yandex,vacancyAI}.ts` → `lib/ai/` (runAgent, промпты-файлы, zod, retry, **redaction policy §12.5**, `ai_runs` со state-машиной с первого вызова); Vacancy Assistant: Discovery-диалог поверх существующего визарда (+JobProfile из docs/dokihel), Structuring со структурированным выводом. Удаление `lawyerChat` |
| PR-9 | 1C AI | Candidate Analyzer (Evidence/Completeness/Consistency + Interview Planner ограниченно) + Guardrails (grounding-фильтр, лексический фильтр, Human Review Gate) + ревью-UI (`evidence_items`, `ReviewFlag`). Запреты §10/§15 v1.1 — тестами |
| PR-10 | 1D Sharing | Разделение bundle/ссылка, password_hash, max_views пакетов, журнал доступов (ip-хэш, ua) + UI журнала владельца, атомарность. Страница `/pkg/[token]` уже есть — расширяется |
| PR-11 | 1D Hardening | Retention jobs (обезличивание отклонённых по сроку компании); антивирус (ClamAV в фоне или API-скан); EXIF-strip при share; PII: login_events → ip-хэш; (опция) enforcement MFA |
| PR-12 | 1D Публичка | Главная «три входа», `/about`, i18n-прогон словарей, E2E три критических пути (загрузка документа / создание вакансии / отклик) — расширение `tests/e2e/`, Lighthouse ≥ 90 |

Критерии приёмки ТЗ §9 покрываются: 1→PR-2, 2→PR-10, 3→PR-8, 4→PR-6, 5–6→PR-9, 7→PR-7, 8→PR-1/3, 9→PR-6, 10→PR-12.

---

## 11. Судьба существующей документации
