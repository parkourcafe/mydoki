...
> **Статус фазы 1: все 13 PR (PR-0…PR-12) выполнены.** Функциональное ядро готово: личный vault (версии/проверки/архив/напоминания), ручная вертикаль найма §17 (вакансия→отклик→Candidate Card→решение), AI-слой с Human Review Gate, sharing с паролем, retention, публичные страницы. **Осознанно отложено под отдельную санкцию** (изменения поведения на общих с Doki.id таблицах): ужесточение RLS `applications` (анти-абьюз `with check(true)`), атомарный `view_count`, доступ рекрутёров через RLS; а также инфраструктурное — антивирус (ClamAV), EXIF-strip (image-lib = новая зависимость), enforcement MFA (AAL2-flow). Все миграции применяются мейнтейнером в обычном порядке; проверено на локальном Postgres, реальный прод-прогон — за мейнтейнером.
