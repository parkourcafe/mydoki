...
## Расхождения «бриф vs схема» — решить

- [ ] **Теги документов.** В брифе поиск «по тегам», в схеме поля `tags` нет.
      Добавлять `tags text[]` или ограничиться `category` + `subtype`?
- [ ] **Водяной знак / запрет скачивания** при шеринге — в брифе как опция «UI»,
      в схеме у `shares` нет. Добавлять поля (`watermark`, `allow_download`) или
      отложить?
- [ ] **Таблица согласий (`consents`).** Медицина — спец-категория ПДн. Нужна
      ли отдельная таблица согласий с более строгим доступом уже в MVP?
...
## Можно сделать дальше (по твоей команде)

В сессии доступны MCP-инструменты Supabase и GitHub. Когда скажешь, могу:

1. **развернуть** отдельный Supabase-проект и **применить** `schema.sql` (миграцией);
2. начать каркас приложения (Next.js + Auth + загрузка в `vault-files`) по
   [MVP-этапам](07-mvp-roadmap.md);
3. сначала закрыть расхождения «бриф vs схема» выше.

Пока ничего из этого не делаю — по договорённости в репо только ТЗ/план.
